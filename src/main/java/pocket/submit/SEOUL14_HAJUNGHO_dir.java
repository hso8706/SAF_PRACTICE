...
public class SEOUL14_HAJUNGHO_dir {
		...
		try {
			...
			while (socket != null) {
				...
				for (int i = 1; i < 6; i++) {
					...
					if(i == 2 || i == 5)continue;
					for (int j = 0; j < 5; j++) {
						double temp_dis_x = Math.abs(balls[i][0] - HOLES[j][0]);
						double temp_dis_y = Math.abs(balls[i][1] - HOLES[j][1]);
						double temp_dis = Math.sqrt((temp_dis_y * temp_dis_y) + (temp_dis_x * temp_dis_x));
						if(temp_dis < minDis){
